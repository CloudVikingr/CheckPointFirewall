...
Describe 'Module Tests' -Tag Unit {
    Context "Module Tests" {
        ...
        It 'root module CheckPointFirewall.psm1 should exist' {
            $PathToModule | Should -Exist
            $? | Should -BeTrue
        } #psm1Exists
        It 'manifest should contain CheckPointFirewall.psm1' {
            $PathToManifest |
            Should -FileContentMatchExactly "CheckPointFirewall.psm1"
        } #validPSM1
